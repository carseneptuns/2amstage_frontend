...
import { Menu, X, Ticket, User, LogOut, Zap } from "lucide-react";
            ...
            <div className="flex flex-col gap-1 px-5 py-4">
              {HOME_LINKS.map((l) => (
                <button
                  key={l.id}
                  onClick={() => goToSection(l.id)}
                  className="rounded-lg px-3 py-3 text-left text-sm font-medium text-mid hover:bg-white/5 hover:text-hi"
                >
                  {l.label}
                </button>
              ))}
              ...
              {isAuthenticated() ? (
                ...
                <>
                  <Link
                    to="/login"
                    onClick={() => setOpen(false)}
                    className="rounded-lg px-3 py-3 text-sm font-medium text-mid hover:bg-white/5 hover:text-hi"
                  >
                    Masuk
                  </Link>
                  <Link
