  ...
  return (
    <div className="flex h-full w-full flex-col bg-surface">
      <div className="flex items-center justify-between gap-2 border-b border-white/10 px-6 py-6">
        ...
        {onClose && (
          <button onClick={onClose} className="rounded-full p-1.5 text-dim hover:bg-white/5 hover:text-hi md:hidden">
            <X className="h-5 w-5" />
          </button>
        )}
      </div>
      ...
      <div className="border-t border-white/10 p-3 space-y-1">
        <Link
          to="/"
          onClick={onNavigate}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-mid transition hover:bg-white/5 hover:text-hi"
        >
