...
export default function BadgeGrid({ badges, editable = false, onToggleVisible, onMove }) {
  if (!badges || badges.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-white/15 py-12 text-center text-sm text-dim">
            ...
            <div className="absolute inset-x-0 top-0 flex items-center justify-between bg-void/60 p-1.5 opacity-0 backdrop-blur-sm transition group-hover:opacity-100">
              <div className="flex gap-1">
                <button
                  onClick={() => onMove?.(i, -1)}
                  disabled={i === 0}
                  className="rounded-md bg-white/10 p-1 text-hi disabled:opacity-30"
                >
              ...
              <button
                ...
                className="rounded-md bg-white/10 p-1 text-hi"
