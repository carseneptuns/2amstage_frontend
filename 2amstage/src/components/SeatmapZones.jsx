  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
      ...
      <div className="rounded-2xl border border-black/10 bg-surface p-5">
            ...
            return (
              ...
              >
                <rect
                  ...
                  style={{
                    filter: selected ? "drop-shadow(0 0 6px rgba(0,113,227,0.35))" : "none",
                  }}
                />
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-black/10 pt-4 text-xs text-mid">
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-stage/40 ring-1 ring-stage" /> Tersedia
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-black/[0.03] ring-1 ring-white/10" /> Habis
          </span>
          <span className="flex items-center gap-1.5">
          ...
          return (
            <motion.div
              ...
              className={`flex items-center justify-between gap-4 rounded-2xl border p-4 transition-all ${
                qty > 0 ? `border-stage/40 bg-stage/5 ring-1 ${color.ring}` : "border-black/10 bg-surface"
              } ${soldOut ? "opacity-50" : ""}`}
            >
              ...
              {!soldOut && (
                <div className="flex shrink-0 items-center gap-3 rounded-full border border-black/10 bg-void/50 px-2 py-1.5">
                  <button
                    type="button"
                    onClick={() => selectCategory(cat.id, Math.max(0, qty - 1))}
                    disabled={qty === 0}
                    className="flex h-7 w-7 items-center justify-center rounded-full text-mid transition hover:bg-black/[0.04] disabled:opacity-30"
                  >
